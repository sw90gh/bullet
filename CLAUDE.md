...
- **Data**: localStorage (`bujo-entries`, `bujo-goals`, `bujo-auto-backup`) + Firebase Firestore (로그인 시)
- **Auth**: Firebase Google Auth (`useAuth()` 훅)
- **Sync**: Firebase Firestore 실시간 동기화 (`useFirestoreSync()` 훅)
...
- Firestore 구조: `users/{uid}/entries`, `users/{uid}/goals` 컬렉션
- **실시간 구독**: Firestore `onSnapshot`으로 원격 변경 실시간 반영
- **병합 전략**: `updatedAt` 기준 last-write-wins
- **초기 동기화**: 로컬 전용 항목 + 로컬이 더 최신인 항목만 diff 업로드
- **오프라인 삭제 추적**: localStorage에 `deletedEntryIds`/`deletedGoalIds` 저장, 재연결 시 동기화
...
BulletJournalApp/
  ...
  src/
    App.tsx               <- 라우팅, 상태관리, DarkModeProvider, 태그바
    main.tsx              <- 엔트리포인트
    firebase/
    ...
    screens/
      DailyScreen.tsx     <- 목록/시간표 뷰, 터치+마우스 드래그, 자동스크롤
      WeeklyScreen.tsx
      MonthlyScreen.tsx
      AnnualScreen.tsx
    ...
    components/
      ...
      EntryRow.tsx        <- 스와이프 (터치+마우스+우클릭)
      EntryModal.tsx      <- 항목 추가/수정 (반복, 다일간 시간숨김, 클리어버튼)
